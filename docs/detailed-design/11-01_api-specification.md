...
**バリデーションルール**
```typescript
const getSalarySlipsQuerySchema = z.object({
  ...
  fields: z.string().regex(/^[a-zA-Z,]+$/).optional(),
...
const createTransactionSchema = z.object({
  ...
  (data) => new Date(data.transactionDate) <= new Date(),
  {
...
const stringValidation = {
  ...
  name: z.string().min(1).max(100).regex(/^[^\x00-\x1F\x7F-\x9F]*$/),
  id: z.string().regex(/^[a-zA-Z0-9_-]+$/),
  currency: z.enum(['JPY', 'USD', 'EUR', 'GBP', 'CNY']),
  
  // HTMLサニタイゼーション
  htmlSafe: z.string().transform(val => sanitizeHtml(val, {
    allowedTags: [],
    allowedAttributes: {}
  })),
