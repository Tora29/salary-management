...
class PdfDataTransformer {
  ...
  private async executeOcr(
    ...
    const averageConfidence = pageResults.reduce(
...
class StockPriceDataTransformer {
  ...
  private assessDataQuality(
    data: StandardizedStockPriceData,
    source: string
  ): DataQuality {
  ...
  async transformInBatches<TInput, TOutput>(
    ...
    for (let i = 0; i < inputData.length; i += batchSize) {
      ...
      try {
        ...
        // バッチ全体が失敗した場合
        errors.push(new BatchTransformError(
      ...
      results.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          if (result.value.success) {
            ...
            errors.push(new TransformError(
    ...
    for (const layer of this.layers) {
      try {
        ...
        errors.push(new ValidationError(
          `Validation layer '${layer.name}' failed`,
          'system',
          error
        ));
      }
...
class SalarySlipBusinessRuleValidationLayer implements ValidationLayer<SalarySlipData> {
  ...
  private calculateTotalEarnings(data: SalarySlipData): number {
    const earningsFields = [
      'baseSalary', 'overtimePay', 'overtimePayOver60', 'lateNightPay',
      'holidayWorkPay', 'fixedOvertimeAllowance', 'transportationAllowance',
      'housingAllowance', 'familyAllowance', 'qualificationAllowance',
      'expenseReimbursement', 'stockPurchaseIncentive', 'bonus', 'otherEarnings'
    ];
