...
class CacheInvalidationService {
  ...
  private setupEventHandlers() {
    ...
    // 株式取引更新時の無効化
    this.eventEmitter.on('stock_transaction.created', async (data: { userId: string, stockId: string }) => {
      await Promise.all([
    ...
    // 株価更新時の無効化
    this.eventEmitter.on('stock_price.updated', async (data: { symbol: string, stockId: string }) => {
      await Promise.all([
...
class BulkOperationService {
      ...
      const batchResults = await prisma.$transaction(async (tx) => {
        ...
        const existingKeys = new Set(
          existingSlips.map(slip => 
            `${slip.userId}-${slip.paymentDate.toISOString()}-${slip.companyName}`
          )
        );
        ...
        const createdSlips = await tx.$queryRaw<Array<{id: string}>>`
          ...
          WHERE (user_id, payment_date, company_name) IN (
            ${Prisma.join(
              newSlips.map(slip => 
                Prisma.sql`(${slip.userId}, ${slip.paymentDate}, ${slip.companyName})`
              )
            )}
          )
  ...
  // 古いデータの一括削除
  async cleanupOldData(retentionDays: number = 365 * 2) { // 2年
  ...
  private calculateNetPay(earnings: Record<string, string>, deductions: Record<string, string>): number {
...
class JobQueueService {
  ...
  private async processMonthlyReportGeneration(data: JobTypes['monthly-report-generation']) {
    ...
    const reportUrl = await this.saveReportToStorage(reportData, data.userId, data.year, data.month);
