...
export class StructuredLogger implements ILogger {
	...
	private async outputToTransports(
		...
		const transportPromises = this.transports
			...
			.map((transport) =>
				transport.write(logEntry).catch((error) => {
					// トランスポートエラーはコンソールに出力
...
export class DataSanitizer {
	...
	sanitizeHeaders(headers: Record<string, string>): Record<string, string> {
		...
		const sensitiveHeaders = new Set(['authorization', 'cookie', 'x-api-key', 'x-auth-token']);
	...
	private findPattern(fieldName: string): PIIPattern | undefined {
		return this.piiPatterns.find(
			(pattern) =>
				pattern.field === fieldName.toLowerCase() || fieldName.toLowerCase().includes(pattern.field)
		);
	}
...
export abstract class BaseTransport implements LogTransport {
	...
	protected setupBuffering(): void {
		if (this.config.bufferSize && this.config.bufferSize > 1) {
			this.flushTimer = setInterval(() => this.flush(), this.config.flushInterval || 5000);
		}
	}
	...
	protected async flush(): Promise<void> {
		...
		try {
			...
			const criticalEntries = entries.filter(
				(entry) => entry.level === 'ERROR' || entry.category === 'security'
			);
	...
	protected async handleCriticalLogFailure(entries: LogEntry[]): Promise<void> {
		...
		const emergencyFile = path.join(process.cwd(), 'logs', `emergency-${Date.now()}.log`);
...
export class ElasticsearchTransport extends BaseTransport {
	...
	private getIndexName(timestamp: string): string {
		...
		return this.indexTemplate.replace('%{+yyyy.MM.dd}', `${year}.${month}.${day}`);
	}
...
export class FileTransport extends BaseTransport {
	...
	private generateFilename(template: string): string {
		return template.replace('%DATE%', this.currentDate).replace('%PID%', process.pid.toString());
	}
...
export class SalarySlipService extends BaseService {
	...
	async createFromPdf(userId: string, files: PdfFile[]): Promise<SalarySlipCreationResult> {
		// 処理開始ログ
