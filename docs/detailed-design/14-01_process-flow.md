...
class SalarySlipQueryService {
	async searchSalarySlips(
		...
		const dbResult = await this.repository.findBySpecification(query.userId, specification);
...
class StockPriceUpdateService {
	...
	private async processBatch(symbols: string[]): Promise<BatchResult> {
		...
		const pricePromises = symbols.map((symbol) =>
			this.fetchStockPrice(symbol).catch((error) => ({ symbol, error }))
		);
		...
		if (validPrices.length > 0) {
			...
			// 影響を受けるポートフォリオの再計算
			const affectedUsers = await this.getAffectedUsers(validPrices.map((p) => p.symbol));
...
class DashboardService {
	...
	private async calculateTrends(userId: string, data: AggregatedData): Promise<TrendAnalysis> {
		// 3ヶ月のトレンドデータ取得
		const trendData = await this.repository.getTrendData(userId, 3);

		return {
			incomeGrowth: this.calculateGrowthRate(trendData.income.current, trendData.income.previous),
...
class RecoveryService {
	async executeWithRecovery<T>(operation: () => Promise<T>, config: RecoveryConfig): Promise<T> {
		let lastError: Error;
