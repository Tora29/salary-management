...
export const verifyGoogleIDToken = async (idToken: string, expectedNonce: string): Promise<GoogleIDToken> => {
  return new Promise((resolve, reject) => {
    jwt.verify(idToken, getGooglePublicKey, {
...
export class CircuitBreaker {
  ...
  private trip(): void {
    ...
    console.warn(`Circuit breaker tripped for ${this.serviceName}. Next attempt at: ${new Date(this.nextAttempt)}`);
  }
...
export class RetryManager {
  ...
  async executeWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
      try {
        ...
        if (!classified.isRetryable) {
          console.error(`${serviceName}: Non-retryable error on attempt ${attempt}:`, error.message);
          throw error;
        }
        ...
        console.warn(`${serviceName}: Attempt ${attempt} failed, retrying in ${delay}ms:`, error.message);
    ...
    throw new ExternalAPIError(
      ...
      `Max retries exceeded for ${serviceName}`,
...
export class AlertManager {
  ...
  private sendAlert(alert: AlertEvent): void {
    ...
    console.error(`🚨 ALERT [${alert.severity.toUpperCase()}] ${alert.serviceName}: ${alert.rule.metric} = ${alert.value} (threshold: ${alert.rule.threshold})`);
      ...
      ctx.json({
        chart: {
          result: [{
            meta: {
              symbol,
              regularMarketPrice: 2820.00,
              previousClose: 2800.00,
              regularMarketDayHigh: 2850.00,
              regularMarketDayLow: 2780.00,
