...
export class ConfigurationManager {
	...
	private loadConfiguration(): ApplicationConfig {
		return {
			...
			stockApi: {
				provider:
					(process.env.STOCK_API_PROVIDER as 'alpha_vantage' | 'yahoo' | 'polygon') ||
					'alpha_vantage',
...
export class PdfProcessingFacade {
	...
	async processPdfFiles(
		...
		try {
			...
			// ステップ7: 結果集計
			const finalResults = this.aggregateResults(validationResults, deduplicationResults, options);
	...
	private async validateFiles(files: PdfFile[]): Promise<FileValidationResult[]> {
		return Promise.all(files.map((file) => this.pdfValidator.validate(file)));
	}
	...
	private async parseAndValidateData(
		...
		return Promise.all(extractionResults.map((result) => this.dataParser.parse(result)));
	}
	...
	private async saveOriginalFiles(processingId: string, files: PdfFile[]): Promise<void> {
		await Promise.all(
			files.map((file) =>
				this.fileStorage.saveFile(`processing/${processingId}/${file.name}`, file.buffer)
			)
		);
	}
...
export class EmailNotificationStrategy implements NotificationStrategy {
	...
	async send(notification: NotificationRequest): Promise<NotificationResult> {
		try {
			...
			// テンプレート適用
			const emailContent = await this.templateEngine.render(this.getTemplate(notification.type), {
				user,
				notification,
...
export class InMemoryEventBus implements EventBus {
	...
	subscribe<T extends DomainEvent>(eventType: string, handler: EventHandler<T>): void {
		...
		// 優先度順にソート
		this.handlers.get(eventType)!.sort((a, b) => a.handler.getPriority() - b.handler.getPriority());
...
export class InMemoryCommandBus implements CommandBus {
	...
	async execute<TResult>(command: Command<TResult>): Promise<TResult> {
		...
		if (!validationResult.isValid) {
			throw new CommandValidationError('Command validation failed', validationResult.errors);
		}
...
export class SalarySlipStateContext {
	...
	public confirm(): void {
		if (!this.state.canConfirm()) {
			throw new InvalidStateTransitionError(this.state.getStatus(), SalarySlipStatus.CONFIRMED);
		}
	...
	public archive(): void {
		if (!this.state.canArchive()) {
			throw new InvalidStateTransitionError(this.state.getStatus(), SalarySlipStatus.ARCHIVED);
		}
...
export class DraftState implements SalarySlipState {
	...
	archive(context: SalarySlipStateContext): void {
		throw new InvalidStateTransitionError(SalarySlipStatus.DRAFT, SalarySlipStatus.ARCHIVED);
	}
...
export class ConfirmedState implements SalarySlipState {
	...
	private validateForArchiving(salarySlip: SalarySlip): void {
		...
		if (daysDiff < 365) {
			// 1年未満はアーカイブ不可
			throw new SalarySlipValidationError('1年以上経過した給料明細のみアーカイブできます');
		}
	}
...
export class EventSourcedSalarySlip extends EventSourcedAggregate {
	...
	public get status(): SalarySlipStatus {
		return this._status!;
	}
	// ... 他のgetters
}
...
export class PatternCoordinationService {
	...
	async executeEventSourcedCommand<TResult>(
		...
		return this.eventStore.withTransaction(async () => {
			...
			const aggregate = await this.loadEventSourcedAggregate(command.aggregateId, aggregateType);
	 ...
	 * CQRS パターンとの統合
	 */
	async executeCqrsOperation<TResult>(operation: CqrsOperation<TResult>): Promise<TResult> {
		if (operation.isQuery()) {
			...
			await this.eventBus.publish(
				new ReadModelUpdateRequiredEvent(operation.command.getType(), operation.command.aggregateId)
			);
	...
	private async sendNotificationsAsync(workflowId: string, result: any): Promise<void> {
		try {
...
export class AttendanceInfoFlyweight {
	...
	public calculateContext(
		...
		const lateNightPay = MoneyAmount.multiply(
			baseSalary,
			(this.lateNightHours * lateNightRate) / 100
		);
