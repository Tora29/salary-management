...
export class CircuitBreakerOpenError extends BaseError {
	...
	constructor(
		...
		super(`Circuit breaker ${circuitBreakerName} is OPEN`, { circuitBreakerName, retryAfter });
	}
...
export class ErrorMessageService {
	constructor(private readonly currentLocale: string = 'ja') {}

	getMessage(category: string, key: string, params: Record<string, string | number> = {}): string {
...
export class ContextualErrorMessageService {
	 ...
	 */
	public generateUserMessage(error: BaseError, context: ErrorDisplayContext): ErrorDisplayMessage {
	...
	private getSuggestedActions(error: BaseError, context: ErrorDisplayContext): ErrorAction[] {
...
export class ErrorPerformanceOptimizer {
	...
	constructor(private readonly config: ErrorPerformanceConfig) {}
	...
	private shouldDebounce(error: BaseError): boolean {
		return error.category === ErrorCategory.VALIDATION || error.severity === ErrorSeverity.LOW;
	}

	private shouldBatch(error: BaseError): boolean {
		return (
			error.category === ErrorCategory.FILE_PROCESSING && error.severity === ErrorSeverity.MEDIUM
		);
	}

	private isUrgentError(error: BaseError): boolean {
		return (
			error.severity === ErrorSeverity.CRITICAL || error.category === ErrorCategory.AUTHENTICATION
		);
	}
...
export class ErrorHandlingTestUtils {
	...
	public static testErrorLocalization(
		...
		return locales.map((locale) => {
			...
			try {
				const message = messageService.getMessage(error.category, error.code.toLowerCase());
