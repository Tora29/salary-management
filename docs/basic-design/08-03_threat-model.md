...
class TamperProtection {
  ...
  static generateIntegrityHash(data: any): string {
    ...
    return crypto
      .createHmac('sha256', process.env.INTEGRITY_KEY!)
      .update(serialized)
      .digest('hex');
  }
...
class PrivilegeProtection {
  // 1. オブジェクトレベル認可
  static async checkObjectAccess(
    user: User,
    resource: Resource,
    action: string
  ): Promise<boolean> {
    ...
    // 追加の条件チェック（時間、IP制限など）
    const contextChecks = await this.checkContextualAccess(
      user,
      resource,
      action
    );
  ...
  // 3. 最小権限の実装
  static applyLeastPrivilege(
    user: User,
    requestedPermissions: string[]
  ): string[] {
    // ユーザーの実際の権限と要求された権限の交差
    const grantedPermissions = requestedPermissions.filter(
