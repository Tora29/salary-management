...
const mobileNavigation: NavigationConfig = {
	...
	drawer: {
		...
		overlay: true,
		swipeToOpen: true,
		items: [...primaryNavItems, 'divider', ...secondaryNavItems]
	},
	bottomBar: {
		enabled: true,
		height: '56px',
...
// 派生ストア
export const canGoBack = derived(navigationStore, ($nav) => $nav.navigationHistory.length > 1);
...
const pageFilters = {
	...
	'/portfolio': {
		filters: [{ key: 'gainLoss', type: 'toggle', options: ['利益', '損失', 'すべて'] }],
		sort: [
			{ key: 'value', label: '評価額' },
