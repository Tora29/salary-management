...
class PortfolioStore extends BaseStore<PortfolioState> {
  ...
  startAutoUpdate(interval = 60000) { // デフォルト1分
    this.stopAutoUpdate();
...
export const validators = {
  ...
  min: (min: number, message = `${min}以上の値を入力してください`) => (value: number) =>
    value >= min ? undefined : message,
  
  max: (max: number, message = `${max}以下の値を入力してください`) => (value: number) =>
...
export function renderWithStores(component: any, props = {}, stores = {}) {
  const mockStores = Object.entries(stores).reduce((acc, [key, value]) => ({
    ...acc,
...
export function memoizedDerived<T, R>(
  stores: any,
  fn: (values: any) => R,
  initialValue?: R
) {
