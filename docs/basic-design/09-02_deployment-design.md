...
export class DatabaseMigration implements MigrationConfig {
	...
	async backup(): Promise<string> {
		...
		await execAsync(
			`supabase db dump --db-url ${process.env.DATABASE_URL} > backups/${backupId}.sql`
		);
...
class RollbackManager {
	...
	private async verifyRollback(): Promise<void> {
		...
		// 基本機能テスト
		const criticalEndpoints = ['/api/auth/session', '/api/salary-slips', '/api/portfolio'];
...
test.describe('Deployment Smoke Tests', () => {
	...
	test('Critical pages are accessible', async ({ page }) => {
		const criticalPages = ['/', '/dashboard', '/salary-slips', '/portfolio'];
