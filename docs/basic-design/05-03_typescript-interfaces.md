 ...
 * 金額の検証スキーマ
 */
const moneyAmountSchema = z.string().regex(/^\d+(\.\d{1,2})?$/, '有効な金額を入力してください');
 ...
 * 収入詳細スキーマ
 */
export const earningsDetailSchema = z
	.object({
 ...
 * 控除詳細スキーマ
 */
export const deductionsDetailSchema = z
	.object({
 ...
 * 給料明細作成スキーマ
 */
export const createSalarySlipSchema = z
	.object({
	...
	.refine((data) => new Date(data.targetPeriodStart) <= new Date(data.targetPeriodEnd), {
 ...
 * 株式取引作成スキーマ
 */
export const createStockTransactionSchema = z
	.object({
