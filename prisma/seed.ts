...
async function main() {
      ...
      assets: {
        create: [
          ...
          },
          {
            ...
            purchasePrice: new Decimal(185.50),
  ...
  console.log(`  - プロファイル: 2件`);
  console.log(`  - 給料データ: 12件（各ユーザー6ヶ月分）`);
  console.log(`  - 資産データ: 10件（各ユーザー5種類）`);
  console.log('\n⚠️  注意: Supabaseの認証ユーザーは別途作成が必要です！');
  console.log('  以下のユーザーをSupabaseダッシュボードから作成してください:');
  console.log('  - Email: test1@example.com (田中太郎)');
  console.log('  - Email: test2@example.com (佐藤花子)');
}
